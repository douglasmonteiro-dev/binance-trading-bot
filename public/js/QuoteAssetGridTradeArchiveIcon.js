...
class QuoteAssetGridTradeArchiveIcon extends React.Component {
  constructor(props) {
    super(props);
    ...
    this.modalToStateMap = {
      ...
      deleteAllByQuoteAsset: 'showDeleteAllByQuoteAssetModal'
    };
  ...
  deleteGridTradeArchive(query) {
    ...
    this.handleModalClose('deleteAllByQuoteAsset');
  ...
  render() {
    const { quoteAsset, isAuthenticated, quoteAssetTickSize } = this.props;
    const {
      loading,
      showArchiveModal,
      rows,
          ...
          <tr>
            <td className={`text-center align-middle`}>
              ...
              <span className='fs-9' title={moment(row.archivedAt).format()}>
                {t('archive.closedTimeAgo', {
                  timeAgo: moment(row.archivedAt).fromNow()
                })}
              </span>
            </td>
            <td
    ...
    return (
      <div className='coin-info-quote-asset-grid-trade-archive-wrapper d-inline-block'>
        <div className='coin-info-column coin-info-column-quote-asset-grid-trade-archive d-flex flex-row justify-content-start align-content-between border-bottom-0 mb-0 pb-0'>
          <button
            type='button'
            className='btn btn-sm btn-link ml-0 mr-1 p-0 text-white line-height-1'
        ...
        <Modal
          ...
          <Modal.Header closeButton className='pt-1 pb-1'>
            <Modal.Title>
              {t('closedTrades.title')} - {quoteAsset}
            </Modal.Title>
          </Modal.Header>
        ...
        <Modal
          show={this.state.showDeleteAllByQuoteAssetModal}
          onHide={() => this.handleModalClose('deleteAllByQuoteAsset')}
          size='md'>
          <Modal.Header className='pt-1 pb-1'>
            <Modal.Title>
              <span className='text-danger'>
          ...
          <Modal.Footer>
            <Button
              variant='secondary'
              size='sm'
              onClick={() => this.handleModalClose('deleteAllByQuoteAsset')}>
              {t('common.cancel')}
            </Button>
            <Button
              variant='danger'
